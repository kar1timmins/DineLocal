...
import { Bell, CheckCircle2Icon, CircleX, InfoIcon, PopcornIcon, TriangleAlert, X } from 'lucide-react';
...
const toastVariants = cva(
  "relative group w-full md:min-w-[364px] md:max-w-[364px] rounded-lg border px-4 py-3 items-center [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-5 shrink-0 [&_svg]:shrink-0 [&_svg]:mb-auto [&_svg]:translate-y-[2px] [&_svg]:text-current",
  {
    variants: {
      variant: {
        default: "bg-card text-card-foreground [&_[data-slot='toast-description']]:text-gray-600 border border-gray-200",
        error:
        ...
        info:
          "text-primary-700 [&_[data-slot='toast-title']]:text-primary-600 [&_[data-slot='toast-description']]:text-primary-600/90 bg-primary-50 border border-primary-200",
      },
    },
    defaultVariants: {
      variant: "default",
...
type ToastProps =  VariantProps<typeof toastVariants> & React.ComponentProps<"div"> & {
  id: any;
  title?: string;
  description?: string;
    ...
    <Box className={cn(toastVariants({variant}), className)}>
        <Flex alignItems={'start'} className=" gap-3">
          ...
          <Box className="flex-1 min-w-0">
            {title && <p data-slot="toast-title" className='font-semibold text-sm'>{title}</p>}
            {description && <p data-slot="toast-description" className='text-sm'>{description}</p>}
      ...
      <Box className='absolute top-1 right-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200 hover:opacity-80 transition-opacity flex-shrink-0'>
